...
export default async function AiTermsRoute({ searchParams }: AiTermsRouteProps) {
  ...
  const terms = await listPublicAiTerms({ locale: "zh", q: query, categorySlug, sort: query ? "latest" : "featured", limit: 48 });

  return <AiTermsPage categorySlug={categorySlug} locale="zh" query={query} terms={terms} />;
}
