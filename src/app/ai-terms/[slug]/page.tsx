...
export async function generateMetadata({ params }: AiTermDetailRouteProps): Promise<Metadata> {
  ...
  const title = term.seoTitle || `${term.term}｜AI 词条｜${siteConfig.name}`;
  const description = term.seoDescription || term.shortDesc;
  const url = `/ai-terms/${term.slug}`;
...
export default async function AiTermDetailRoute({ params }: AiTermDetailRouteProps) {
  ...
  const blocks = await parseLayeredMarkdown(term.contentMd, "zh");
  const tocItems = buildArticleToc(blocks);

  return <AiTermDetailPage blocks={blocks} locale="zh" term={term} tocItems={tocItems} />;
}
