...
export async function POST(request: Request) {
  ...
  try {
    const invalidArticles = invalidArticleResponse(await validateSeriesArticleIds(parsed.data.locale, parsed.data.articleIds));
    if (invalidArticles) {
      return invalidArticles;
    }

    await createSeries(parsed.data);
    return json(await adminSeriesPayload(), { status: 201 });
  } catch (error) {
    return databaseError(error);
  }
}
