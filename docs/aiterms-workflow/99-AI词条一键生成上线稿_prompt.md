...
- 没有明确要求时，只生成 `pro`。
- “生成一图看懂”：生成图片提示词。
- “生成本地图 / 生成图片 / 生成一图看懂本地图”：当前LLM有图片生成能力，则生成本地图。
- “优化图片 / 图片优化 / imageOptimize”：使用项目中的脚本把一图看懂本地图处理为100KB 以内的 WebP；优先输出 1600×900，若压缩后无法满足 100KB 以内，再降级为 1280×720。
- “生成寓言故事”：生成独立寓言故事素材。
...
- “term=XXX，一条龙。”等同于完整流程：生成 `pro`、一图看懂prompt、本地图、图片优化、寓言故事，并同步测试环境 D1/R2 草稿；其中本地图仍受上述图片生成边界约束。
...
```
   ...
   如果图片生成能力不可用、限流、无法保存本地图，则保留 图片提示词，跳过本地图、图片优化和同步，并说明原因。
...
12. 对于已明确为“补齐/修正少量素材产物”的小任务，优先走快速路径：先复用现有 pro、prompt 和本地图；只补缺失文件与必要验证，不展开完整批量生产流程，不生成冗长报告，除非用户明确要求。
