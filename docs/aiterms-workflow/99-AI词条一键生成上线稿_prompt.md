...
- Skill 不是本流程的默认执行工具；除非用户明确点名某个 skill，或本项目文档/脚本明确指定，否则按项目本地工作流执行。
...
```
   ...
   如果图片生成能力不可用、限流、无法保存本地图，或只能通过未授权 skill / 外部 API / 第三方脚本完成，则保留 brief 和图片提示词，跳过本地图、图片优化和同步，并说明原因。
