...
assert.match(formatSearchAuditMarkdown(searchAudit), /发布前清单/);
assert.match(formatSearchAuditMarkdown(searchAudit), /llms\.txt 片段/);
assert.match(formatSearchAuditPublishMarkdown(searchAudit), /AI 可引用摘要/);
assert.match(formatSearchAuditPublishMarkdown(searchAudit), /JSON-LD/);
assert.doesNotMatch(formatSearchAuditPublishMarkdown(searchAudit), /AI 搜索体检结果/);
assert.equal(searchAudit.extracted.evidenceSignals.links, 1);
assert.ok(searchAudit.extracted.evidenceSignals.datedFacts >= 1);
...
const lateAnswerAudit = analyzeSearchAudit("# AI 搜索优化\n\n这是一段很短的导语。\n\n这是另一段没有直接回答目标问题的铺垫。\n\nAI 搜索优化需要先把答案放在开头，再提供事实来源、案例和结构化数据。", {
  targetKeyword: "AI 搜索优化",
  targetQuestion: "应该如何优化 AI 搜索？",
});
assert.ok(lateAnswerAudit.issues.some((issue) => issue.id === "aeo-direct-answer"));
assert.match(lateAnswerAudit.faq[0]?.answer ?? "", /建议用 40-80 字/);
