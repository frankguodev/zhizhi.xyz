...
- 确认 `.env*`、真实 `wrangler.toml`、Wrangler token、数据库 ID、后台账号密码和本地路径没有提交。
- 确认公开仓库只保留 `wrangler.example.toml`，真实配置只留在本地或部署平台 secret 中。
- 检查 `docs/`、`summery/`、`HISTORY.md` 中是否包含邮箱、账号、截图、隐私路径或未脱敏对话。
- 确认 `LICENSE`、`README.md`、`.gitignore`、`package-lock.json` 都已准备好。
...
- 测试环境先执行 `npm run db:migrate:test`，确认 D1 migration 全部应用成功。
- 测试环境创建或提升后台管理员账号。
- 在测试环境跑一遍：登录后台、导入 Markdown、保存草稿、编辑草稿、上传图片、发布文章、挂载专题、外链展示。
- 检查匿名点赞、阅读次数、文章可见性、媒体读取和后台权限。

### 前台和 SEO

- 检查首页、文章列表、文章详情、专题页、关于页、工具页在桌面和移动端的布局。
- 检查 `/sitemap.xml`、`/robots.txt` 能正常访问。
- 检查文章 metadata：`title`、`description`、`robots`、Open Graph、Twitter 图片、canonical 和中英文 alternates。
- 检查 `/media/articles/*` 图片能正常访问，并确认响应包含长期缓存头。
- 检查登录可见文章、`noindex` 文章和公开文章的访问策略是否符合预期。
- 测试部署稳定后，再把生产站点添加到 Google Search Console，并提交 sitemap。

### 生产发布

- 将生产 D1、R2、Worker、secret 和 `wrangler.toml` 配置核对一遍。
- 对生产 D1 执行 migration 前，确认测试环境已经通过。
- 执行生产部署后，按测试环境同样的链路做一次抽样回归。
- 记录本次发布版本、迁移、验证结果和遗留风险。

## 安全与隐私

- 不要提交 `.env.local`、Wrangler secret、Cloudflare token、数据库真实生产 ID 或个人凭据。
- 不要提交真实 `wrangler.toml`。公开仓库保留 `wrangler.example.toml` 即可；如果真实文件已经被追踪，发布前需要从 Git 索引移除。
- 后台账号应使用强密码，生产环境建议叠加 Cloudflare Access。
- 匿名点赞、阅读次数和反馈使用匿名设备 Cookie 降低重复计数和滥用，不保存读者姓名或账号资料。
- 文章图片和社交分享图是公开资源，代码层使用边缘缓存和长期缓存头降低 R2 读取压力；生产环境建议在 Cloudflare WAF 中为 `/media/articles/*` 增加宽松限速。
- 前台普通用户系统当前软下线，保留数据结构和部分代码，后续需要时再恢复。
- 文章 frontmatter 中涉及 `source`、`human_reviewed`、`robots` 等字段，发布前应人工复核。
- 公开仓库前请检查 `summery/`、`docs/` 和历史提交中是否包含隐私信息、账号、路径、截图或密钥。

## 编码规范

所有文本文件必须使用 UTF-8 without BOM。

原因：Windows PowerShell 5.1 的 `Set-Content -Encoding utf8` 会写入 UTF-8 BOM，可能影响 JSON、Markdown、构建工具或脚本解析。
...
- 优先使用编辑器保存为 UTF-8 without BOM。
- 在自动化脚本中使用 Node `fs.writeFileSync(path, content, "utf8")`。
- 避免 PowerShell 5.1 的 `Set-Content -Encoding utf8`。
