...
- seo_title：MCP 是什么？模型上下文协议的通俗解释
- seo_description：MCP（Model Context Protocol）是连接 AI 应用与外部工具、数据源和工作流的开放协议。本文用小白能理解的方式解释 MCP 的作用、架构、使用场景和常见误解。
- seo_keywords：MCP、Model Context Protocol、模型上下文协议、AI Agent、AI Coding、工具调用、AI 连接器、JSON-RPC

### 分类候选

- primary_category：ai-protocol
- secondary_category：ai-agent
- category_reason：MCP 的核心是 AI 应用和外部工具/数据源之间的协议与互操作机制，因此主分类应为 `ai-protocol`；它常用于 Agent 场景，副分类可选 `ai-agent`。
...
- visual_goal：让读者一眼理解 MCP 是连接 AI 应用与外部工具、数据源和工作流的协议层，不是模型本身。
- visual_metaphor：统一接口 / 插线板 / USB-C 连接器。
- visual_structure_suggestion：连接器型。
- key_elements：AI 应用、MCP Client、MCP Server、Tools、Resources、Prompts、外部系统、用户授权。
- short_labels：AI 应用、统一协议、工具调用、读取资源、提示模板、外部系统、授权边界。
- boundary_warning：MCP 不自动保证安全，工具和数据访问仍需要授权与权限控制。
- do_not_draw：不要画官方 Logo；不要暗示 MCP 是模型；不要画成万能自动化或无权限调用。
- image_alt_seed：一张手绘图展示 MCP 如何把 AI 应用通过统一协议连接到工具、资源、提示模板和外部系统。
...
- uncertain_points：不同平台对 MCP 的支持范围、远程/本地 MCP 能力边界、安全实践和规范版本可能持续变化。
