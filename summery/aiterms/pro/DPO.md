...
DPO 不需要先显式训练奖励模型，但仍然需要偏好数据。偏好可以来自人工标注、规则筛选或模型辅助标注，但来源质量会直接影响训练结果。

### 误解二：DPO 完全替代 RLHF

DPO 是一种简化偏好优化流程的方法，常被拿来和 PPO 式 RLHF 对比。它在很多场景里更容易训练，但不是所有对齐问题的唯一答案。

### 误解三：DPO 训练后模型一定更安全

DPO 可以让模型更贴近偏好数据中的目标，但偏好数据本身未必覆盖所有安全问题。安全性还取决于数据设计、拒答策略、评估和部署护栏。

### 误解四：DPO 只是在做普通监督微调

监督微调通常学习一个目标回答本身；DPO 学的是偏好回答相对不偏好回答的优势，并借助参考模型控制偏移。
